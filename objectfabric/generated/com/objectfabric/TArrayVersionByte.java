 ...
 */
class TArrayVersionByte extends TIndexedNVersion {

    private byte[][] _values;

    private static final TType[] _genericParameters = new TType[] { new TType(ImmutableClass.BYTE) };

    private static final boolean CAN_BE_TOBJECT = false;

    public TArrayVersionByte(TObject.Version shared, int length) {
        super(shared, length);
    ...
    public TObject.Version merge(TObject.Version target, TObject.Version next, int flags) {
        TArrayVersionByte source = (TArrayVersionByte) next;
        TArrayVersionByte merged = (TArrayVersionByte) super.merge(target, next, flags);
        merged.merge(source, flags);
        return merged;
    }

    @SuppressWarnings("cast")
    private final void merge(TArrayVersionByte source, int flags) {
    ...
    private final void merge(byte[] merged, com.objectfabric.misc.Bits.Entry writes, byte[] source) {
        for (int i = com.objectfabric.misc.Bits.BITS_PER_UNIT - 1; i >= 0; i--)
            if (com.objectfabric.misc.Bits.get(writes.Value, i))
                merged[i] = source != null ? source[i] : ((byte) 0);
    }
    ...
    public TObject.Version createVersion() {
        return new TArrayVersionByte(this, length());
    }
