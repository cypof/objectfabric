...
public final class Transaction extends TransactionPublic implements SystemClass {
    ...
    private static final AtomicIntegerFieldUpdater<Transaction> _statusUpdater;
    ...
    private static final AtomicReferenceFieldUpdater<Transaction, Session> _currentSessionUpdater;
    ...
    static {
        ...
        _statusUpdater = AtomicIntegerFieldUpdater.newUpdater(Transaction.class, "_status");
        _currentSessionUpdater = AtomicReferenceFieldUpdater.newUpdater(Transaction.class, Session.class, "_currentSession");
    }
            ...
            if (!transaction.isCommitted()) {
                for (;;) {
                    ...
                    if (status == STATUS_SUSPENDED) {
                        if (_statusUpdater.compareAndSet(transaction, STATUS_SUSPENDED, STATUS_DEFAULT))
                            break;
                    } else {
                        if (Debug.ENABLED)
        ...
        synchronized (shared) {
            ...
            if (reference instanceof Descriptor)
                ...
                for (;;) {
                    ...
                    _currentSessionUpdater.compareAndSet(this, session, created);
                }
            }
