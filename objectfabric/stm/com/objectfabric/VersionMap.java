...
import com.objectfabric.misc.ThreadAssert.SingleThreadedOrShared;
...
final class VersionMap {
    ...
    private static final AtomicIntegerFieldUpdater<VersionMap> _watchersUpdater;
    ...
    private static final AtomicIntegerFieldUpdater<VersionMap> _mergeInfoUpdater;
    ...
    static {
         ...
         */
        _watchersUpdater = AtomicIntegerFieldUpdater.newUpdater(VersionMap.class, "_watchers");
        _mergeInfoUpdater = AtomicIntegerFieldUpdater.newUpdater(VersionMap.class, "_mergeInfo");
    }
    ...
    public boolean tryToAddWatchers(int count) {
        ...
        for (;;) {
            ...
            if (_watchersUpdater.compareAndSet(this, watchers, newWatchers))
                return true;
        }
    }
    ...
    private boolean mergeAndReturnIfShouldWalkDelayedQueue(Transaction branch, Snapshot mapSnapshot) {
         ...
         */
        if (!_mergeInfoUpdater.compareAndSet(this, MERGE_DEFAULT, MERGE_A)) {
        ...
        if (!_mergeInfoUpdater.compareAndSet(b, MERGE_DEFAULT, MERGE_B)) {
            if (Debug.ENABLED) {
                mapSnapshot = mapSnapshot != null ? mapSnapshot : Helper.TO_MERGE_LATER_IS_NULL;
                Debug.assertion(Helper.getInstance().getToMergeLater().put(this, mapSnapshot) == null);
            }
    ...
    public void onAborted() {
        if (_transaction != null)
            if (_mergeInfoUpdater.compareAndSet(this, MERGE_DEFAULT, MERGE_DONE))
