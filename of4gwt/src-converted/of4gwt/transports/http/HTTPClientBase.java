...
import of4gwt.misc.AtomicBoolean;
...
abstract class HTTPClientBase extends Connection {
    ...
    protected HTTPClientBase(final Object url, Validator validator) {
        super(Site.getLocal().getTrunk(), null, validator);

        _transport = new CometTransport() {

            @Override
            protected HTTPRequestBase createRequest(boolean serverToClient) {
                return HTTPClientBase.this.createRequest(url, serverToClient);
            }
            ...
            protected void read(byte[] buffer, int offset, int limit) {
                ...
                HTTPClientBase.this.read(buffer, offset, limit);
            ...
            protected int write(byte[] buffer, int offset, int limit) {
                ...
                int written = HTTPClientBase.this.write(buffer, offset, limit);
    ...
    public void connect() throws IOException {
        ...
        try {
            future.get();
        } catch (of4gwt.misc.InterruptedException ex) {
            throw new RuntimeException(ex);
        } catch (of4gwt.misc.ExecutionException ex) {
    ...
    public Future<Void> connectAsync(AsyncCallback<Void> callback, AsyncOptions asyncOptions) {
        ...
        sendHeartbeatEvery(CometTransport.CLIENT_HEARTBEAT);
        enableTimeout(CometTransport.CLIENT_TIMEOUT);

        return future;
    }

    public Object connectAndWaitObject() {
