...
import com.objectfabric.TSetExtended;
...
public abstract class ExtendedTester {
    ...
    public static final int REUSE_MAP_BY_CLEAR = 1 << 1;

    public static final int REUSE_MAP_BY_REMOVES = 1 << 2;
    ...
    public void run() {
        ...
        test(getTest(1, REUSE_MAP_BY_CLEAR), CYCLES);
        test(getTest(1, REUSE_MAP_BY_REMOVES), CYCLES);
    ...
    public void run2Threads() {
        ...
        test(getTest(2, REUSE_MAP_BY_CLEAR), CYCLES);
        test(getTest(2, REUSE_MAP_BY_REMOVES), CYCLES);
    ...
    public static void threadBefore(ExtendedTest test) {
    ...
    public static Object getCachedOrProxy(int flags, Object cached, Object wrapper) {
        boolean cleared = false;

        if ((flags & REUSE_MAP_BY_CLEAR) != 0) {
            clear(cached);
            cleared = true;
        } else if ((flags & REUSE_MAP_BY_REMOVES) != 0) {
        ...
        try {
            if (test.threadCount() == 1) {
                ...
                run(test.threadCount(), new Runnable() {
                    ...
                    public void run() {
                        int index = PlatformAdapter.getRandomInt(methods.size());
    ...
    private static void invoke(Method method, ExtendedTest test) {
        test.threadBefore();
    ...
    public static void main(String[] args) throws Exception {
        ExtendedTester extendedTest = new TSetExtended();
        TestsHelper.disableExpectedExceptionThrowerCounter();

        for (int i = 0; i < 100; i++)
            extendedTest.test(extendedTest.getTest(2, PROXY_1), CYCLES);
    }
}